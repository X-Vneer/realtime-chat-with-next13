...
const AddFrindsForm = (props: Props) => {
    ...
    // form state
    const { register, handleSubmit, setError, formState: { errors, isSubmitting, isLoading } } = useForm<FormData>({
        ...
        <section className='p-8'>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="flex w-full max-w-sm  items-start space-x-2">
                    ...
                    <Button type="submit" className=' whitespace-nowrap' isLoading={isSubmitting}>Add Friend</Button>
                </div>
                {errors.root ? <p className='mt-1 text-[12px] text-red-600'>{errors.root.message}</p> : null}
                {showSuccessState ? (
